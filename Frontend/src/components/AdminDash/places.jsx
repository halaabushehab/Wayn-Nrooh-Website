...
//             if (parsedUser.isAdmin) {
//               axios.defaults.headers.common['Authorization'] = `Bearer ${parsedUser.token}`;
//               console.log("User is an admin. Token added to headers.");
//             } else {
//               console.log("User is not an admin.");
//             }
//           }
...
//         useEffect(() => {
//           setPagination(prev => ({...prev, page: 1})); // العودة للصفحة الأولى عند تغيير الفلتر
//         }, [filter]);
//         // setStats({
//         //   topPlaces: [],
//         //   byCategory: []
...
//         }
//       );
//       console.log("Places coming from API:", placesRes.data);

//       setPlaces(places.map(p => p._id === id ? { ...p, status } : p));
...
//   const handleApprove = async (id) => {
//     try {
//       await axios.patch(`http://localhost:9527/dashboard/places/${id}/status`, 
//         { status: 'approved' }, 
...
//     try {
//       await axios.delete(`http://localhost:9527/dashboard/delete/${id}`, {
//         headers: { Authorization: `Bearer ${localStorage.getItem('token')}` }
//       });
//       setPlaces(places.filter(p => p._id !== id));
...
//   const handleEdit = (id) => {
//     // navigate(`http://localhost:9527/dashboard/update/${id}`);
...
//       const url = `http://localhost:9527/dashboard/places?page=${pagination.page}&limit=${pagination.limit}&search=${searchQuery}${
//         filter !== 'all' ? `&status=${filter}` : ''
//       }`;
...
//         <div>
//           <h1 className="text-2xl font-bold text-[#022C43]">إدارة الأماكن</h1>
//           <p className="text-gray-500">إدارة جميع الأماكن في النظام</p>
//         </div>
//         <button 
//           className="bg-[#115173] hover:bg-[#022C43] text-white px-4 py-2 rounded-lg flex items-center transition-colors"
//           onClick={() => navigate('/admin/places/adpaginationd')}
//         >
//           <Plus size={18} className="ml-2" />
//           إضافة مكان جديد
//         </button>
...
//           {/* Pagination */}
//           {places.length > 0 && pagination.totalPages > 1 && (
//             <div className="mt-8 flex justify-center">
//               <div className="flex items-center gap-2">
//                 <button 
//                   className="w-10 h-10 flex items-center justify-center rounded-lg border border-[#115173] bg-white text-[#115173] hover:bg-[#115173] hover:text-white disabled:opacity-50 disabled:cursor-not-allowed"
